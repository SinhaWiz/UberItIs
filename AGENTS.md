...
- **6. Driver Payment Notification**: `notification-service` is now implemented and consumes the `payment.completed` event, but `PaymentCompletedEvent` (as published by `payment-service`) only carries `riderId` — no `driverId` — so only the rider gets notified today. Notifying the driver requires `payment-service` to add `driverId` to the event payload first.
